...
const ConfirmDeleteDialog = ({ exhibitor, onConfirm, onClose }) => {
  ...
  return (
    <Modal isOpen onClose={onClose} title="Delete Exhibitor?" size="sm">
      <div className="p-6 space-y-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm">
                ...
                <ul className="list-disc list-inside space-y-1 text-destructive/80">
                  <li>All associated collection data will be removed</li>
                  <li>Login credentials will be invalidated</li>
                  {exhibitor.activeMovies > 0 && (
                    <li>Unassigns {exhibitor.activeMovies} active movie{exhibitor.activeMovies > 1 ? 's' : ''}</li>
                  )}
                  {exhibitor.totalCollections > 0 && (
                    <li>Archives ₹{exhibitor.totalCollections.toLocaleString('en-IN')} in collections</li>
                  )}
                </ul>
              </div>
            </div>
        ...
        <div className="bg-muted/50 rounded-md p-4 text-sm text-muted-foreground space-y-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-medium text-foreground">Exhibitor Details</span>
          </div>
      ...
      <div className="flex items-center gap-3 p-6 border-t border-border">
        ...
        <Button variant="destructive" onClick={onConfirm} iconName="Trash2" iconPosition="left" className="flex-1">
          Delete Exhibitor
        </Button>
      </div>
    </Modal>
